...
2. **Detect KVM** — if `/dev/kvm` is readable *and* writable (and the node isn't itself a VM, see the nested-virtualization note), QEMU runs with `-enable-kvm -cpu host`; otherwise it falls back to software emulation with `-cpu max`, which exposes every TCG-emulatable feature — including x86-64-v2, which EL9-family guests (Rocky/Alma 9) hard-require. Disk I/O always uses `cache=writeback,aio=threads`. Either way the VM boots.
